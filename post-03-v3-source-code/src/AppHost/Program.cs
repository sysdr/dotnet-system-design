...
// Fixed host ports match docker-compose.yml (8080/8081). Run .\stop.ps1 before
// restarting — a stale DCP proxy from a prior session blocks these ports.
...
builder.AddContainer("otel-collector",
    ...
    .WithEndpoint(
        ...
        name: "grpc",
        scheme: "http",
        isExternal: false)   // not browsable, only receives OTLP
    .WithEndpoint(
        port: 55679,
        targetPort: 55679,
        name: "zpages",
