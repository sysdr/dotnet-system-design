...
1. Click **Structured Logs** in the left sidebar. Trigger a login:
   ```powershell
   # Aspire 9: /api/v1/resources is auth-gated (returns HTML without login).
   # Ports are fixed in launchSettings.json — do not scrape the Dashboard API.
   Invoke-RestMethod -Method Post -Uri "http://localhost:5080/users/login"
   ```
