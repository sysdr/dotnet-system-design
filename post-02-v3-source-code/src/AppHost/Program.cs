...
const string collectorOtlpGrpc = "http://localhost:4317";

var userService = builder.AddProject<Projects.UserService>("user-service")
    .WithExternalHttpEndpoints()
    .WithEnvironment("OTEL_EXPORTER_OTLP_LOGS_ENDPOINT", collectorOtlpGrpc);
...
builder.AddContainer("otel-collector",
    ...
    .WithEndpoint(
        ...
        name: "grpc",
        scheme: "http",
        isExternal: false)   // not browsable, only receives OTLP gRPC
    .WithEndpoint(
        port: 4318,
        targetPort: 4318,
        name: "http",
        scheme: "http",
        isExternal: false)   // App Insights IngestionEndpoint=http://localhost:4318/
    .WithEndpoint(
        port: 55679,
        targetPort: 55679,
        name: "zpages",
        scheme: "http",
        isExternal: true);   // browse http://localhost:55679/debug/pipelinez
